...
'''
parser.add_argument('--path', nargs='?', const=1, default='test/', help='Test image folder', type= str)
...
root = 'E:/idao/track_1/idao_dataset/'
model = load_model(root+'weights.hdf5')
...
for filename in tqdm(os.listdir(root + test_FOLDER)):
    if not filename.endswith('.png'):
        continue

    path = os.path.join(root+test_FOLDER,filename)
